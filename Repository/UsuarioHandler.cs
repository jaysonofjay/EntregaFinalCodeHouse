...
namespace EntregaFinal.Repository
{
    public class UsuarioHandler : DbHandler
    {
        ...
        public static bool LoginUser(string nombreUsuario, string enter_pword)
        {
            //    //Usuario usuario = new Usuario();
            string current_pw;
            using (SqlConnection sqlConnection = new SqlConnection(ConnectionToMyServer))
            {
                var queryPWord = "SELECT u.Contraseña FROM Usuario u WHERE NombreUsuario = @nombreUsuario;";
                var parametroUser = new SqlParameter("nombreUsuario", SqlDbType.VarChar);
                parametroUser.Value = nombreUsuario;

                sqlConnection.Open();
                using (SqlCommand comandoPW = new SqlCommand(queryPWord, sqlConnection))
                {
                    comandoPW.Parameters.Add(parametroUser);
                    current_pw = Convert.ToString(comandoPW.ExecuteScalar());
                }
                sqlConnection.Close();
            }

            if (enter_pword == current_pw)
            {
                return true;
            }
            else
